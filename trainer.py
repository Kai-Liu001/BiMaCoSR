...
class TrainerBase:
    ...
    def build_model(self):
        ...
        if self.num_gpus > 1:
            self.model = DDP(model.cuda(), device_ids=[self.rank,], broadcast_buffers=False,find_unused_parameters=True)  # wrap the network
        else:
    ...
    def train(self):
        ...
        ###############################################  quant and load
        if self.configs.quantparameter.train.quant_before_load is False :
            self.load_model(self.model,self.configs.quantparameter.train.pretained_model)
        if  self.configs.quantparameter.train.if_quant is True:
            if self.num_gpus==1:
                self.model.replace_conv_and_linear()
            else:
                self.model.module.replace_conv_and_linear()

        if self.configs.quantparameter.train.quant_before_load :
            self.load_model(self.model,self.configs.quantparameter.train.pretained_model)
        ...
        for ii in range(self.iters_start, self.configs.train.iterations):
            ...
            if 'val' in self.dataloaders  :
                if (ii+1)%self.configs.train.get('val_freq', 10000) == 0:
                    self.validation()
    ...
    def save_ckpt(self,str=None):
        if self.rank == 0:
            ckpt_path = self.ckpt_dir / 'model_{:d}.pth'.format(self.current_iters)
            if str is not None:
                    ckpt_path = self.ckpt_dir / 'best.pth'.format(self.current_iters)
            ...
            if hasattr(self, 'ema_rate'):
                ...
                if str =="best":
                    ema_ckpt_path = self.ema_ckpt_dir / 'best.pth'.format(self.current_iters)
                elif str =="last":
                    ema_ckpt_path = self.ema_ckpt_dir / 'last.pth'.format(self.current_iters)
                elif str =="psnr":
                    ema_ckpt_path = self.ema_ckpt_dir / 'psnr.pth'.format(self.current_iters)
                elif str =="lpips":
                    ema_ckpt_path = self.ema_ckpt_dir / 'lpips.pth'.format(self.current_iters)
...
class TrainerDistillDifIR(TrainerDifIR):
    def __init__(self, configs):
        ...
        self.learn_xT = configs.train.get("learn_xT", False)#inverse loss
        self.reformulated_reflow = configs.train.get("reformulated_reflow", False)
        self.finetune_use_gt = configs.train.get("finetune_use_gt", False)#一致性损失
    ...
    def build_model(self):
        params = self.configs.model.get('params', dict)
        params_teacher = self.configs.model_teacher.get('params', dict)
        
        heterogeneous_model = False
        if params_teacher is None: params_teacher = params
        else: heterogeneous_model = True
        for key, value in params_teacher.items():
            print(f"{key}: {value}")
        teacher_model = util_common.get_obj_from_str(self.configs.model_teacher.target)(**params_teacher)
        
        if self.num_gpus > 1:
            self.teacher_model = DDP(teacher_model.cuda(), device_ids=[self.rank,], broadcast_buffers=False if not self.uncertainty_hyper else True, find_unused_parameters=True)  # wrap the network
        else:
        ...
        if self.rank == 0:
            ...
            if (self.current_iters % self.configs.train.log_freq[0] == 0 or self.current_iters == 1) and flag:
                ...
                for key, val in self.loss_mean.items():
                    ...
                    if key == 'loss':
                        self.train_losses.append(val[0].item())
                        self.steps.append(self.current_iters)

                log_str += 'lr:{:.2e}'.format(self.optimizer.param_groups[0]['lr'])
                self.logger.info(log_str)
                self.log_step[phase] += 1

                if self.loss_mean['loss'][0].item() < self.minloss:
                    self.minloss = self.loss_mean['loss'][0].item()
                    self.save_ckpt("best")
    ...
    def validation(self, phase='val'):
        ...
        if self.rank == 0:
            ...
            if 'gt' in data:
                ...
                mean_lpips /= len(self.datasets[phase])
                self.logger.info(f'Validation Metric: PSNR={mean_psnr:5.2f}, LPIPS={mean_lpips:6.4f}...')
                self.val_losses.append(mean_lpips)
                self.val_psnr.append(mean_psnr)
                self.val_lpips.append(mean_lpips)
                self.log_step[phase] += 1
           # print(self.current_iters,"psnr","seccess")
            if mean_lpips < self.minlpips:
                self.minlpips = mean_lpips
                self.save_ckpt("lpips")
                print("lpips best model saved")
            if mean_psnr > self.maxpsnr:
                self.maxpsnr = mean_psnr
                self.save_ckpt("psnr")
                print("psnr best model saved")
            self.logger.info("=" * 100)
            if not self.configs.train.use_ema_val:
                self.model.train()
            torch.cuda.empty_cache()
