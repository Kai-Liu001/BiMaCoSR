...
class BinaryConv2d_BBCU(nn.Conv2d):
    def __init__(self, in_chn, out_chn, kernel_size=3, stride=1, padding=1, bias=True,pretained_weight=None,if_lora=True,if_skip=False):
        super(BinaryConv2d_BBCU, self).__init__(
            in_chn,
            out_chn,
            kernel_size,
            stride=stride,
        ...
        self.move0 = LearnableBias(in_chn)
        self.binary_activation = SignSTE_Bireal()
        self.relu=RPReLU(out_chn)
    ...
    def forward(self, x):
        ...
        binary_weights = self.weight_quantize(real_weights, N=1)
        out = F.conv2d(out, binary_weights, self.bias, stride=self.stride, padding=self.padding)
        out =self.relu(out)
        if out.size() == x.size():
            out = out + x
        return out

def init_BinaryConv2d_BBCU_from_conv(conv):
    binary_conv = BinaryConv2d_BBCU(conv.in_channels, conv.out_channels, conv.kernel_size[0], conv.stride[0], conv.padding[0], conv.bias is not None)
    binary_conv.weight = conv.weight
    if conv.bias is not None:
        binary_conv.bias = conv.bias
    return binary_conv

class BinaryLinear_BBCU(nn.Linear):
    def __init__(self, in_features, out_features, bias=True, binary_act=True):
        super(BinaryLinear_BBCU, self).__init__(in_features, out_features, bias=bias)
...
def init_BinaryLinear_BBCU_from_Linear(linear):
    binary_linear = BinaryLinear_BBCU(linear.in_features, linear.out_features, linear.bias is not None)
