...
class BaseSampler:
    ...
    # diffusion model
        ...
        if self.configs.quantparameter.inference.quant_before_load and self.configs.quantparameter.inference.if_quant:
            model.replace_conv_and_linear()
        ...
        if self.configs.quantparameter.inference.quant_before_load is False and self.configs.quantparameter.inference.if_quant:
            model.replace_conv_and_linear()
    ...
    def load_model(self, model, ckpt_path=None):
        ...
        print(f"Loading model from {ckpt_path} on rank {self.rank}")
        if 'state_dict' in state:
            state = state['state_dict']

        util_net.reload_model_print(model, state)
