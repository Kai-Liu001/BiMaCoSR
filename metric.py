...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the ground truth (GT) images directory."
    )
    ...
    parser.add_argument(
        "--metrics",
        nargs="+",
        default=['PSNR', 'SSIM', 'LPIPS', 'DISTS', 'CLIPIQA', 'CLIPIQA+', 'NIQE', 'MUSIQ', 'MANIQA','FID'],
        help="List of metrics to evaluate. Defaults to all metrics."
    )
    ...
    iqa_metrics = {name: available_metrics[name] for name in args.metrics if name in available_metrics}
    logger.info(f"Metrics to be evaluated: {', '.join(iqa_metrics.keys())}")
    logger.info("IQA metrics initialized.\n")

    # Validate input and GT directories
    gt_dir = args.gt_imgs
    for init_dir in args.inp_imgs:
    ...
    for init_dir in args.inp_imgs:
        ...
        for img_idx, sr_path in enumerate(img_sr_list):
            ...
            with torch.no_grad():
                ...
                for name, metric in iqa_metrics.items():
                    if name=='FID':
                        continue
                    elif name in ['CLIPIQA', 'CLIPIQA+', 'NIQE', 'MUSIQ', 'MANIQA']:
        ...
        # Compute FID for the directory if selected
        if 'FID' in iqa_metrics:
            fid_start_time = time.time()
            fid_value = iqa_metrics['FID'](gt_dir, init_dir).item()
            fid_end_time = time.time()
            fid_runtime = fid_end_time - fid_start_time
            avg_metrics['FID'] = fid_value
            avg_metrics['FID Runtime'] = fid_runtime
