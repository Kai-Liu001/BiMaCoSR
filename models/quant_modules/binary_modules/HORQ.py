...
class BinaryConv2d_HORQ(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=True):
        super(BinaryConv2d_HORQ, self).__init__(
            in_channels,
            out_channels,
            kernel_size,
            stride=stride,
    ...
    def forward(self, x):
        ...
        x=HORQ_grad_activation(x, N=2, oursign=self.quantize)
        # Weight quantization and convolution
        real_weights = self.weight
        binary_weights = HORQ_grad(real_weights, N=2)
        x = F.conv2d(x, binary_weights, self.bias, stride=self.stride, padding=self.padding)
        return x

def init_BinaryConv2d_HORQ_from_conv(conv):
    binary_conv = BinaryConv2d_HORQ(conv.in_channels, conv.out_channels, conv.kernel_size[0], conv.stride[0], conv.padding[0], conv.bias is not None)
...
class BinaryLinear_HORQ(nn.Linear):
    def __init__(self, in_features, out_features, bias=True):
        super(BinaryLinear_HORQ, self).__init__(in_features, out_features, bias=bias)
        self.quantize = SignSTE_clip()
    def forward(self, input):
        # Activation quantization
        input=HORQ_grad_activation(input, N=2, oursign=self.quantize)
...
def init_BinaryLinear_HORQ_from_Linear(linear):
    binary_linear = BinaryLinear_HORQ(linear.in_features, linear.out_features, linear.bias is not None)
